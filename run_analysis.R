...
train <- fread(file.path(path, "UCI HAR Dataset/train/X_train.txt"))[, desiredfeatures, with = FALSE]
...
# TRAIN <- TrainSubjects--TrainActivity--Train
train <- cbind(trainSubjects, trainActivity, train)

# Loading Test Datasets
test <- fread(file.path(path, "UCI HAR Dataset/test/X_test.txt"))[, desiredfeatures, with = FALSE]
...
# Test <- TestSubjects--TestActivity--Test
